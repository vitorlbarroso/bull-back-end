...
class CelCashController extends Controller
{
    ...
    public function generate_payment_pix(GeneratePaymentPixRequest $request)
    {
        ...
        if ($getPrincipalOffer->product->user->cash_in_adquirer_name == 'super') {
            ...
            $generatePayment = CelCashService::generatePaymentPixBySuper($data, $unicId);

            if (isset($generatePayment['data']['id'])) {
                $unicId = $generatePayment['data']['id'];
                $pixReference = $generatePayment['data']['pixDetail']['emv'];

                $adquirerName = 'super';

                $returnData = [
                    'galax_pay_id' => $generatePayment['data']['id'],
                    'qr_code' => $generatePayment['data']['pixDetail']['emv'],
                    'upsell' => $getPrincipalOffer->sale_completed_page_url
                ];
            } else {
                return Responses::ERROR('Ocorreu um erro ao gerar o pedido!', $generatePayment, 1400, 400);
        ...
        if ($getPrincipalOffer->product->user->cash_in_adquirer_name == 'medius') {
            $unicId = "BP_ID_" . Str::upper(Str::random(30));

            $data['customer']['document']['number'] == '00000000000' ? $data['customer']['document']['number'] = '39233341097' : null;

            $generatePayment = CelCashService::generatePaymentPixByMedius($data, $unicId);
            if (isset($generatePayment['id'])) {
                $unicId = $generatePayment['id'];
                ...
                $returnData = [
                    'galax_pay_id' => $generatePayment['id'],
                    'qr_code' => $generatePayment['pix']['qrcode'],
                    'upsell' => $getPrincipalOffer->sale_completed_page_url
                ];
            } else {
                return Responses::ERROR('Ocorreu um erro ao gerar o pedido!', $generatePayment, 1400, 400);
        ...
        try {
            DB::transaction(function () use ($request, $getPrincipalOffer, $pixReference, $generatePayment, $totalPrice, $calculateTax, $formatedPayday, $validatedData, $offersData, $adquirerName, $unicId) {
                $createCelcashPayments = CelcashPayments::create([
                    ...
                    'buyer_name' => $validatedData['customer_name'] ?? 'Bulls Pay',
                    'buyer_email' => $validatedData['customer_email'] ?? 'compras@bullspay.com.br',
