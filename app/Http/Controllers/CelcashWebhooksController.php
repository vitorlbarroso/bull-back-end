...
class CelcashWebhooksController extends Controller
{
    ...
    public function transactions_medius(Request $request)
    {
        $validatedData = $request->validate([
            'type' => 'required',
            'data' => 'required',
        ...
        if ($getTransaction->type == 'pix') {
            ...
            if (
                $validatedData['data']['status'] == 'waiting_payment'
            ) {
                $status = 'pending_pix';
                $isPayedStatus = false;
            }
            ...
            /* if (
                $validatedData['data']['status'] == 'chargedback'
            ) {
                $status = 'chargeback';
                $isPayedStatus = false;
            } */
    ...
    public function transactions_owen(Request $request)
    {
        $validatedData = $request->validate([
            'object' => 'required',
            'object.id' => 'required',
        ...
        $getTransaction = CelcashPayments::where('galax_pay_id', $validatedData['object']['id'])
            ->with('payment_offers', function($query) {
                $query->where('type', 'principal')
                    ->with('offer', function($query) {
                        $query->with('product', function($query) {
        ...
        if ($getTransaction->type == 'pix') {
            if (
                $validatedData['object']['status'] == 'paid'
            ) {
                $status = 'payed_pix';
                $isPayedStatus = true;
            }
            ...
            if (
                $validatedData['object']['status'] == 'pending'
            ) {
                $status = 'pending_pix';
                $isPayedStatus = false;
            }
        ...
        if ($isPayedStatus) {
            ...
            if (!$buyerUser) {
                ...
                $data['card_tax_value'] = $configsData->default_card_tax_value ?? 4.99;
                $data['card_money_tax_value'] = $configsData->default_card_money_tax_value ?? 1.5;
                $data['cash_in_adquirer_name'] = $configsData->default_cash_in_adquirer ?? 'owen';
                $data['cash_out_adquirer_name'] = $configsData->default_cash_out_adquirer ?? 'owen';
            ...
            try {
                Mail::to($buyerUser->email)->send(new BuyerMail($buyerUser->name, $buyerUser->email, $getTransaction->payment_offers[0]->offer->product->email_support, $validatedData['object']['id']));
            }
            catch (\Exception $e) {
                Log::error("|" . request()->header('x-transaction-id') . '| Ocorreu um erro ao tentar enviar um e-mail de pagamento |', [ 'ERRO' => $e->getMessage()]);
            }
        ...
        $pendingEvents = PendingPixelEvents::where('payment_id', $validatedData['object']['id'])
            ->where('status', 'Waiting Payment')
            ->get();
        ...
        if($getTransaction->payment_offers[0]->offer->utmify_token) {
            ...
            try {
                $body = [
                    "orderId" => $validatedData['object']['id'],
                    "platform" => "BullsPay",
                    "paymentMethod" => "pix",
                    "status" => "paid",
                    "createdAt" => $getTransaction->created_at,
